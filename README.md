
A partir de https://mystery.knightlab.com/ criamos na disciplina de banco de dados uma dinâmica de trabalho onde os alunos poderiam criar os próprios "mistérios SQL". Este repositório é um cartão de visita desses trabalhos, que os próprios alunos subiram publicamente em seus repositórios.
São alunos de 3º período de graduação de Ciência de Dados e IA da FGV - EMAp, Escola de Matemática Aplicada.

Os mistérios foram preparados para uso via github codespaces ou localmente.

# Mistério SQL: A Alavancagem Fantasma - Mercado Financeiro

Um jogo interativo de investigação de dados baseado no modelo de banco de dados real do nosso grupo. Desenvolvido em Python e Streamlit, o sistema lê diretamente as abas de uma planilha Excel e simula um ambiente de consultas SQL (JOINs, filtros, cruzamento de chaves) voltado para cenários de auditoria e Compliance no mercado financeiro.

https://github.com/adrianfilipe/misterio_sql

# Mistério no Expresso do Oriente: Um Crime em SQL - da obra de Agatha Christie

 "Para resolver um mistério, mon ami, não precisamos apenas de olhos atentos, mas de mentes brilhantes trabalhando em perfeita sincronia. Permita-me apresentar a equipe de investigadores que enviou os comandos SQL mais precisos de toda a Europa." — Hercule Poirot

https://github.com/JorMartinelli/MisterioSQL

# Um desafio investigativo de SQL inspirado no clássico SQL Murder Mystery.

https://github.com/Jose-Pedro-Dresch/bd_sql_mystery

# João Ômega e o saqueador de gabaritos

A história do nosso mistério está presente no tanto no arquivo HTML quanto no docs abaixo.
Link do docs:
https://docs.google.com/document/d/13gaBcD4gnvJ2nKJT75tukyrjy0JaF3z5PhWQHoUaIiU/edit?tab=t.0

https://github.com/pedroalves085/ProjetoBD

# Mistério SQL — Operação Fantasma da Previdência

Mistério SQL é um jogo investigativo em dois episódios onde você assume o papel do Agente Lucas Mendonça (PF-2026-11492), um agente federal em seu primeiro caso de campo. O crime foi registrado em 17/05/2026 às 23h19: alguém usou o CPF de um homem morto para fazer uma reserva no Airbnb.

Sua missão é resolver o caso escrevendo queries SQL.

https://github.com/jhe-sua/bd-mistery-sql

# O Roubo da Pizza
Na calada da noite, um crime assolou a Pizzaria da cidade: o roubo misterioso de uma pizza.

Tudo começou com uma reclamação trivial de suporte. Um cliente faminto ligou para o estabelecimento enfurecido: seu pedido estava atrasado há horas. Ao checar o sistema, o atendente tomou um susto — o rastreamento indicava que a pizza já havia saído para a entrega fazia muito tempo. O entregador simplesmente sumiu do mapa, e a pizza nunca chegou ao destino.

https://github.com/renzovecchioni/Misterio-SQL

# Joalheiro encontrado morto em Pintópolis-MG

Herbert Rodrigues, um joalheiro muito rico e famoso, foi encontrado morto em Pintópolis-MG. Conhecidos da vítima alegaram que o homem teria viajado para a cidade em busca de pedras preciosas para serem utilizadas na confecção de um anel de noivado para o Vini Jr., que iria não só tentar voltar com a Virgínia, mas também pedi-la em casamento. Isso foi confirmado pelo depoimento de uma vendedora de joias, que afirmou ter entregado em mãos para a vítima aproximadamente R$ 1.000.000 em diamantes na manhã do dia 08/05. Essa foi a última vez que a vítima foi vista, e a autópsia confirmou a causa mortis como morte matada.

https://github.com/d4ruh/SQL-murder-mystery